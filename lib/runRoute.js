...
module.exports=function(req,res){
  ...
  if(moduleName.indexOf("api/")<0){//不是api接口请求
    ...
    if(config.apiLogShow) console.log("api请求("+(apiCount+=1)+")："+(typeof config.apiProxy==="string" ? config.apiProxy : config.apiProxy.protocol+"//"+config.apiProxy.host+":"+config.apiProxy.protocol)+moduleName);
