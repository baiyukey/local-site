...
let getFileDir=function(_path){
  return _path.replace(/^(.*)([\\\/])(.*)$/,"$1");// /a/b/c.js ==> /a/b/c
};
...
let htmlToJsCssPath=function(_sourcePath){
  ...
  if(config.html.buildJsCss===0){
    ...
    jsPath=jsCssSourceDir+'js/'+samePath.replace(/^(.*)(\/)(.*)$/,"$1$2js/$3")+".js";//返回d:/www/uielf.com/media/js/a/index.js
    cssPath=jsCssSourceDir+'css/'+samePath.replace(/^(.*)(\/)(.*)$/,"$1$2css/$3")+".css";//返回d:/www/uielf.com/media/css/a/index.css
  }
  return {
    "jsPath":jsPath,
    "cssPath":cssPath
...
let thisWatch=function(_dir){
  ...
  watch.watchTree(_dir,function(_filePath,_curr,_prev){//filePath是当前的文件, curr是该文件的当前状态对象，prev是前一个状态对象
    ...
    if(config.jsCss.isUglify && FileExtReg.test(fileExt) && sourcePath.includes(jsCssSourceDir) && !sourcePath.includes(`min${fileExt}`)){
      executeCssJs(sourcePath,_curr,_prev);
    }
    else if([0,1].includes(config.html.buildJsCss) && FileExtReg.test(fileExt) && sourcePath.indexOf(htmlDir)>=0){
