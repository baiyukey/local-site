...
function run(){
  ...
  let createCallback=function(req,res){
    ...
    if(url.parse(req.url).pathname==="/") thisPath=config.templateDir+"index.html";
    else if(url.parse(req.url).pathname.split("/").pop()==="") thisPath=config.templateDir+"index.html";
