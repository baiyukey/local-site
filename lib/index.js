...
function run(){
  ...
  let httpEvent=function(req,res){
    let thisPath=(dir+config.htmlRoot+url.parse(req.url).pathname).replace(/\/\//g,"/");
    if(url.parse(req.url).pathname.replace(/.*\//,'').replace(/[?#].+/,'')==="") thisPath+="index.html";
    if(["htm","html"].indexOf(thisPath.replace(/^(.*\/.*\.)(.*)$/,"$2"))<0) thisPath=dir+config.filesRoot+url.parse(req.url).pathname;
