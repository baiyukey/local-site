...
function run(){
  ...
  let httpEvent=function(req,res){
    let thisPath=(config.htmlDir+url.parse(req.url).pathname).replace(/\/\//g,"/");
    if(url.parse(req.url).pathname.replace(/.*\//,'').replace(/[?#].+/,'')==="") thisPath+="index.html";
    if(url.parse(req.url).pathname.split("/")[1]==="media") thisPath=config.rootDir+url.parse(req.url).pathname;
